...
#| echo: false
#| message: false
#| label: fig-scatter-plot-reasons
#| fig-cap: Relationship Between Reasons for and Items Found During Per Strip Search in Toronto, 2020-2021

#create a scatterplot to show each observation in the dataset, comparing the number of reasons to the items found in each strip search
...
cleaned_search_reasons_data |>
  ...
  count() |>
  rename(count = n) |>
  ggplot(aes(x = total_reasons, y = count, colour = items_found)) +
  geom_bar() +
  theme_bw() +
  theme(legend.key.size = unit(0.5, 'cm'))+
  labs(x = "Total Reasons for Strip Searching",
...
cleaned_race_gender_data |>
  ggplot(aes(x = income, y = count, label = ward_code)) + 
  geom_point() + 
  geom_beeswarm() +
