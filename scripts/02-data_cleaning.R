...
cleaned_arrests_strip_data <-
  ...
  select(-c(x_id, arrest_id, person_id, object_id, booked, actions_at_arrest_concealed_i, actions_at_arrest_combative, actions_at_arrest_resisted_d, actions_at_arrest_mental_inst, actions_at_arrest_assaulted_o, actions_at_arrest_cooperative)) |> # Get rid of unnecessary variables
  rename(
...
# Replace all "XX" and "None" from the columns arrest_loc_div and items_found with "NA" respectively to make the data processing more efficient later on
...
cleaned_race_gender_data$strip_search[is.na(cleaned_race_gender_data$strip_search)] <- "No"
cleaned_race_gender_data$strip_search[cleaned_race_gender_data$strip_search == 1] <- "Yes"
